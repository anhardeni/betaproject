...
def get_data(filters):
    ...
    item_group = filters.get("item_group") or "Finished Goods"
    ...
    for idx, item in enumerate(item_list, start=1):
        ...
        # In custom inventory report, usually we skip items with zero activity AND zero balance
    ...
    # Re-index because of skipped rows
...
def get_opname_data(from_date, to_date, company, warehouse):
    ...
    if company:
        ...
        params["company"] = company
    sql = f"""
        SELECT sri.item_code, sri.qty as physical_qty
