...
def get_opening_balances(from_date, company, warehouse):
    """Calculate opening balance for all items before from_date."""
...
def get_movement_data(from_date, to_date, company, warehouse):
    """Categorize movements into Pemasukan, Pengeluaran, Adjustment in period."""
    ...
    for r in res:
        ...
        if code not in summary:
            summary[code] = {"in": 0, "out": 0, "adj": 0}
            
        if vtype == "Stock Reconciliation":
            summary[code]["adj"] += qty
        elif qty > 0:
            summary[code]["in"] += qty
        else:
            summary[code]["out"] += abs(qty)
            
    return summary


def get_opname_data(from_date, to_date, company, warehouse):
    """Get the latest physical count from Stock Reconciliation in the period."""
    params = {"from_date": from_date, "to_date": to_date}
    ...
    # We join with the child table (Stock Reconciliation Item)
    sql = f"""
        SELECT 
            sri.item_code,
    ...
    # Keep only the latest one per item
    opnames = {}
    for r in res:
        if r.item_code not in opnames:
            opnames[r.item_code] = flt(r.physical_qty)
