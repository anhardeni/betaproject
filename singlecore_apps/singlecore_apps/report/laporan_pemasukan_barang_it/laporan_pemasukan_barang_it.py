...
def get_purchase_receipt_data(from_date, to_date, bc_type, supplier_filter, item_code_filter):
    ...
    # SQL HYBRID: Ambil Kuantitas Asli PR, Ambil Nomor Register Asli CEISA Log
    sql = """
        SELECT
            pr.posting_date                                         AS tanggal,
            COALESCE(pr.custom_bc_document_type, '')                AS data_dok_pabean,
            
            -- TARIKAN OTOMATIS DARI LOG BEA CUKAI
            COALESCE(csl.nopen, 'Belum Ada Nopen')                  AS no_daftar,
            COALESCE(csl.nopen_date, pr.posting_date)               AS tgl_daftar,
        ...
        FROM
            ...
            -- PINTU GERBANG KE ARAH CEISA API (JEMBATAN INTEGRASI)
            LEFT JOIN `tabCustoms Status Log` csl ON csl.no_aju = pr.custom_no_aju
...
def get_stock_entry_data(from_date, to_date, bc_type, item_code_filter):
    ...
    sql = """
        ...
        FROM
            ...
            -- PINTU GERBANG KE ARAH CEISA API
